...
`registerType: "prompt"` (not the default `"autoUpdate"`, which reloads silently and never fires `onNeedRefresh`) pairs with [src/scripts/pwa.js](src/scripts/pwa.js), which imports `registerSW` from `virtual:pwa-register` and wires it to the existing `#notification`/`#reload` markup: `onNeedRefresh` shows the banner, and clicking `#reload` calls `updateSW(true)` to activate the new service worker and reload. `injectRegister` is left at its default (`"auto"`), which detects the manual `virtual:pwa-register` import and skips generating the redundant `registerSW.js`. `initPwa()` is called from [index.js](src/scripts/index.js).

When a new build is deployed, a tab left open picks up the new service worker and shows the "new version available" notification banner (`#notification`) with a Reload button.
